...
test('cloud cache reports fresh collection and fresh provider quota', async () => {
    ...
    const file = await cacheFile({ version: 1, generatedAt, providerRegistry: ['drime-1'], providers: { 'drime-1': { account: 'Luke primary', quota: { total: 100, used: 40, free: 60, trashed: 1 }, reachability: 'ok', lastSuccess: generatedAt } } });
    const result = await readCloudCache(file, now, 9 * DAY);
    assert.equal(result.state, 'fresh'); assert.equal(result.collectedAt, generatedAt); assert.equal(result.providers[0].quotaState, 'fresh'); assert.equal(result.providers[0].account, 'Luke primary');
});
